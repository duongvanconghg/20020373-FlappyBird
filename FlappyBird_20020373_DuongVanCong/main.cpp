...
bool InitData()
{
    ...
    SDL_SetHint(SDL_HINT_RENDER_SCALE_QUALITY, "1"); // thiet lap ti le va chat luong

    g_window = SDL_CreateWindow("FlappyBird 20020373 Duong Van Cong",
...
bool LoadBackground()
{
    bool ret = g_background.LoadImageFile("img//bk3.png", g_screen);
    return ret;
}
...
int main(int argc, char* argv[])
{
    ...
    while (!quit)
    {
        ...
        //ve thanh hien diem// Draw Geometric
        ...
        str_time += str_val;
        time_games.SetText(str_time);
        time_games.RenderText(g_screen, SCREEN_WIDTH - 200, 2);
        //hien diem
        time_games.loadFromRenderedText(g_font_text, g_screen);
        std::string score_games = "Score : ";
        int count = manage_block.GetCount();
        std::string count_str = std::to_string(count);
        score_games += count_str;
        text_count_.SetText(score_games);
        text_count_.loadFromRenderedText(g_font_text, g_screen);
        text_count_.RenderText(g_screen, SCREEN_WIDTH * 0.45, 2);
